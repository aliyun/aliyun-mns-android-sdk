...
public class OSSUtils {
     ...
     * 判断一个域名是否是cname
     */
    public static boolean isCname(String host, List<String> customCnameExludeList) {
        for (String suffix : OSSConstants.DEFAULT_CNAME_EXCLUDE_LIST) {
            if (host.toLowerCase().endsWith(suffix.toLowerCase())) {
                return false;
            }
        }
        for (String suffix : customCnameExludeList) {
            if (host.toLowerCase().endsWith(suffix.toLowerCase())) {
                return false;
            }
        }
    ...
    public static String determineContentType(String initValue, String srcPath, String toObjectKey) {
        if (!OSSUtils.isEmptyString(initValue)) {
            return initValue;
        }
    ...
    public static void signRequest(RequestMessage message) throws IOException {
        if (message.getCredential() == null) {
            throw new IllegalStateException("当前Credential为空！！！"
                    + "\n1. 请检查您是否在初始化OSSService时设置Credential;"
                    + "\n2. 如果您bucket为公共权限，请确认获取到Bucket后已经调用Bucket中接口声明ACL;");
        }

        Credential credentialProvider = message.getCredential();
        OSSFederationToken federationToken = null;
        if (credentialProvider instanceof StsTokenCredential) {
            federationToken = ((StsTokenCredential) credentialProvider).getFederationToken();
